...
for (dist_, params) color, ax in zip(distributions, ["C0", "k", "C1"], axes.ravel()):
    for α in [1, 0.5, 2]:
        dist = dist_(*params(α))
...
_, axes = plt.subplots(3, 1, sharex=True, figsize=(10, 7))

for α, ax in zip([0.5, 1, 2], axes.ravel()):
    a_prior = α*2 - α + 1
    b_prior =  α*5 - α + 1
    pz.Beta(a_prior, b_prior).plot_pdf(legend=False, ax=ax, color="C4")
