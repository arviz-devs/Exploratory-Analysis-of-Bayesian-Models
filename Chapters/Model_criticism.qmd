# Model Criticism {#sec-model-criticism}
...
### Prior and likelihood sensitivity with power-scaling

The method we will discuss is detailed in [@kallioinen_2023], the practical advantage of this method is that we can fit a model once, and without explicitly modifying the prior, we can approximate the effect of changing its strength, in comparison to the likelihood. In this way we can easily assess the prior sensitivity. The method allows to do the same with the likelihood.

A key idea of the method is the concept of power-scaling. By power-scaling we mean raising a distribution to a power $\alpha$. As we can see from @fig-beta-normal-power-scaling the effect of power-scaling is to "stretch" or "compress" the distribution. This transformation is very general and it will work for any distribution, except for the uniform distribution.
...
In the context of Bayes Theorem we can use power-scaling to modulate the relative weights of the likelihood and prior. For instance we can write a power-scaled version of Bayesian theorem as follows:

$$
p(\theta \mid y)_{\alpha} \propto p(y \mid \theta)^\alpha \; p(\theta)
$$

If we set $\alpha=1$ we recover the usual expression for Bayes' theorem that probably you have seen many times. Setting $\alpha=0$ is equivalent to ignoring the likelihood, thus $p(\theta \mid y)_{\alpha}$ will be equivalent to the prior. We can then conclude that any number between 0 and 1 will have the effect of "weakening" the likelihood with respect to the prior. By the same token any number greater than 1 will "strength" the likelihood with respect to the prior. Thus, by adjusting the value of $\alpha$ we can obtain a stronger or weaker likelihood, if we use values close to 1, we emulate relative small perturbations. This very same idea can be applied to the prior. @fig-posterior-power-scaling shows the effect on the posterior of power-scaling the prior for a fixed likelihood.
...
We can analytically compute the effect of power scaling a distribution. For instance, for a Normal distribution, $\mathcal{N}(\theta \mid \mu, \sigma)$ and power-scaling factor $\alpha$ we get $\mathcal{N}(\theta \mid \mu, \sigma \; \alpha^{-1/2})$. But as we usually work with MCMC samples and arbitrary distributions a more convenient way to perform power-scaling in practice is to use importance sampling. As discussed in @sec-model-comparison importance sampling is a method to estimate the expected value of a function under a distribution different from the one we have samples from. @kallioinen_2023 proposed to use Pareto smoothed importance sampling (PSIS), a method we also discussed in @sec-model-comparison to estimate the ELPD.

In the context of power-scaling perturbations, the proposal distribution is the posterior we computed using MCMC ($p(\theta \mid y)$) and the target distribution is the perturbed posterior ($p(\theta \mid y)_{\alpha}$). If the proposal and target distributions are expressed as the products of the prior $p(\theta)$ and likelihood $p(y \mid \theta)$, with one of these
components raised to the power of $\alpha$, then we can write the importance weights as:
...
Which means that the importance sampling weights for the prior only depend on the density of the prior being power-scaled. This σ
simplifies the computation of the weights. The same applies to the likelihood.

To sumarize, the method proposed by @kallioinen_2023 combines two key ideas, importance sampling and ower-scaling. This combination allow us to approximate the sensitivity of the posterior to the prior and likelihood without the need to explicitilly build and fit multiple models.

Once we have perturbed the prior and likelihood we can visually inspect the effect of the perturbations, by plotting the marginal posterior distributions for the parameter of interest. We can also compute numerical values based on distances (or divergences) between the unperturbed and perturbed posteriors. In @sec-model-comparison we discussed the use of the Kullback-Leibler divergence to compare two distributions. @kallioinen_2023 instead suggest using the cumulative Jensen–Shannon divergence (CJS) [@Nguyen_2015]. This divergence has two good features, its symmetrised form is upper-bounded, which aids interpretation, and to compute it we can use the empirical cumulative distribution functions (ECDFs), which can be efficiently estimated from MCMC draws. Other methods requieres the estimation of the density of the distributions, which can be more computationally expensive and depends on the choice of the kernel and bandwidth.

The authors suggest a cutoff value of 0.05 for the CJS. For a standard normal this value corresponds to the mean being shifted by more than $\approx 0.3$ standard deviations, or the standard deviation differing by a factor greater than $\approx 0.3$. This value is not set in stone, but it can be used as a reference to interpret the results of the sensitivity analysis.

In the next section we are going to show two examples of on how to use this method in practice.

### Example of prior and likelihood sensitivity analysis
...
We can now use the `psense_summary` function from ArviZ to perform the prior sensitivity analysis. This function returns a four-column DataFrame. The first column is the name of the variables, the second is the value of CJS when power-scaling the prior, then the value of CJS when power-scaling the likelihood and the last one is a textual description of the results.

We can see that for our example, there may be a prior-data conflict for the `wrist` variable. 

```{python}
azs.psense_summary(idata_bf_00, var_names=["~μ"], round_to=2)
```

XX PROVIDE MORE DETAILS ABOUT THE possible combinations of CJS and diagnostic messages.

We can also evaluate the effect of power-scaling visually with the function `plot_psense_dist`. This function allow plot the posterior marginal using KDEs, ECDFs and other representation.
...
Now, let try with a different prior. We will use empirically scaled to the data, $βk \sim \mathcal{N}(0, 2.5 \frac{s_y}{s_{xk}})$, where $s_y$ is the standard
deviation of $y$ and $s_{xk}$ is the standard deviation of predictor variable $xk$.


XXX continue with the example!

### Final remarks about prior (and likelihood) sensitivity analysis
