...
For the comparison we can use visual and numerical summaries as we already explained in previous sections. For instance, when working with ArviZ, we can use `az.plot_forest` or `az.plot_density`, as both plots allows us to compare multiple posteriors in a single plot. Ideally, we should summarize and report the results of this analysis, so that others can also understand how robust the model is to different prior choices.
...
The method we will discuss was presented by @kallioinen_2023, the practical advantage of this method is that we can fit a model once, and without explicitly modifying the prior, we can approximate the effect of changing its strength, in comparison to the likelihood. In this way we can easily assess the prior and/or likelihood sensitivity. If you are eager to see how this method is applied in practice, you can jump to the next section. If you want to understand the method in more detail, keep reading.
...
If we set $\alpha=1$ we recover the usual expression for Bayes' theorem that you may have seen many times. Setting $\alpha=0$ is equivalent to ignoring the likelihood, thus $p(\theta \mid y)_{\alpha}$ will be equal to the prior. We can then conclude that any number between 0 and 1 will have the effect of "weakening" the likelihood with respect to the prior. By the same token any number greater than 1 will "strength" the likelihood with respect to the prior. Thus, by adjusting the value of $\alpha$ we can obtain a stronger or weaker likelihood. If we want to emulate relative small perturbations, then we should use values close to 1. This very same idea can be applied to the prior, @fig-posterior-power-scaling shows the effect on the posterior of power-scaling the prior for a fixed likelihood.
...
To sumarize, the method proposed by @kallioinen_2023 combines two key ideas, importance sampling and power-scaling. This combination allow us to approximate the sensitivity of the posterior to the prior and likelihood without the need to explicitilly build and fit multiple models.
