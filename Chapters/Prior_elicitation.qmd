...
for key, val in refs.items():
    pc.map(axvline, value=val, color="gray", linestyle="--", lw=1)
```
...
```{python}
pc = azp.plot_ppc(idata, group="prior_predictive", kind="ecdf")
for key, val in refs.items():
    pc.map(axvline, value=val, color="gray", linestyle="--", lw=1)
```
...
On the last step, we use a second approximation, we fit the projected posterior into standard distributions used by PPLs as building blocks. We need to do this so we can write the resulting priors in terms a PPLs like PyMC, PyStan could understand. 
