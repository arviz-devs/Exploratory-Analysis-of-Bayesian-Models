...
for i in pz.DiscreteUniform(0, 900).rvs(3, random_state=rng):
    ...
    bad_sample1[0,i:i+100] = pz.Beta(i, 150).rvs(100, random_state=rng)
...
In ArviZ by calling the function `azp.plot_trace_dist(.)` we get a trace plot on the right and on the left a KDE (for continuous variables) or a histogram (for discrete variables). @fig-tracedistplot_good_sample is an example of this. The KDE/histogram can help to spot differences between chains, ideally, distributions should overlap. 
...
azp.plot_trace_dist(sample, var_names="bad_sample_0", compact=True);
```

Finally from @fig-traceplot_bad_sample_1, we can see another common scenario. This time we see that globally everything looks fine. But there are 3 regions where the sampler gets stuck, see the orange bands across the traceplot.

```{python}
#| label: fig-traceplot_bad_sample_1
#| code-fold: true
#| fig-cap: "Trace plot showing a sampler being stuck in some regions (see orange lines)"
pc = azp.plot_trace_dist(sample, var_names="bad_sample_1", compact=True)
...
@fig-rank_plot shows rank plots for `good_sample`, `bad_sample_0` and `bad_sample_1`. 
...
Finally, we want to mention that we can use $\Delta$-ECDF plots to visually inspect any quantity that is expected to be uniformly distributed, not just the ranks. For example, in @sec-ppc we use them to compare the posterior predictive distribution to the observed data (see @fig-post_ppc_pit_ecdf_alt).
...
The following example shows two things; the `non_centered` model shows several divergences (turquoise circles) grouped in one region. In the `centered` model, which has no divergence, you can see that around that same region, there are samples for smaller values of `tau`. That is to say, the `non_centered` model fails to sample a region, but at least it warns that it is having problems sampling that region!
