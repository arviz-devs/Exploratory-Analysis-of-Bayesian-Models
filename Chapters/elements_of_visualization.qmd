...
The color palettes in ArviZ were designed with the help of [colorcyclepicker](https://colorcyclepicker.mpetroff.net). Other pallettes distributed with ArviZ are `'arviz-cetrino'`, and `'arviz-vibrant'`. To list all available styles use `azp.style.available()`.

If you need to do plots in grey-scale we recommend restricting yourself to the first 3 colors of the ArviZ palettes ("C0", "C1" and "C2"), otherwise, you may need to use different [line styles](https://matplotlib.org/api/_as_gen/matplotlib.lines.Line2D.html#matplotlib.lines.Line2D.set_linestyle) or [different markers](https://matplotlib.org/api/markers_api.html#module-matplotlib.markers).
