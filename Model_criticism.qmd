...
There are a few variations of rootograms. Here we are going to discuss hanging rootograms and suspended rootograms. For the first one the observed counts are plotted as dots and the expected counts are plotted as bars hanging from the dots, see @fig-rootogram_hang. 
...
![Bayesian rootograms](img/rootogram_hanging.png){#fig-rootogram_hang width=70%}

A variant of the hanging rootogram is the suspended rootogram (see @fig-rootogram_sus). The difference is that the predictions are plotted starting from 0 (the dashed line), and they go up to the predicted-observed counts. Thus if the line is above 0 the model is overestimating that count and if the line is below 0 the model is underestimating that count.


![Bayesian rootograms](img/rootogram_suspended.png){#fig-rootogram_sus width=70%}
